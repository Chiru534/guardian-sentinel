...
def run_comparative_evaluation():
    ...
    x_test = tf.keras.preprocessing.sequence.pad_sequences(sequences, maxlen=MAX_LENGTH, padding='post')
    ...
    for name, path in MODEL_FILES.items():
        if os.path.exists(path):
            ...
            print(f"Model: {name:<10} | Test Accuracy: {results[1]*100:.2f}% | Test Loss: {results[0]:.4f}")
