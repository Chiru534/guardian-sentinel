...
def test_sync_and_safe_inbox(page: Page):
    ...
    expect(page.locator("button").filter(
        has_text="Quarterly Update Meeting")).to_be_visible(timeout=10000)
    page.locator("button").filter(has_text="Quarterly Update Meeting").click()

    expect(page.get_by_text(
        "Team, please find the quarterly results")).to_be_visible()
...
def test_quarantine_and_explainable_ai(page: Page):
    ...
    expect(page.get_by_role("heading", name="🛡️ Quarantine")
...
def test_kill_switch(page: Page):
    ...
    # Kill switch
    kill_btn = page.get_by_role(
        "button", name="🗑️ Confirm Threat & Delete from Gmail")
    expect(kill_btn).to_be_visible()
    kill_btn.click()

    # Verify success
    expect(page.get_by_text(
        "Neutralized: Threat moved to Gmail Trash.")).to_be_visible()
    expect(page.locator("button").filter(
        has_text="URGENT")).not_to_be_visible()
